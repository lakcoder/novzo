...
if(isset($_POST['prayaasform'])){
  ...
  if($name=="" || $id=="" || $enrollment=="" || $department=="" || $section=="" || $room=="" || $phone==""){
      $msg1 = "Please fill all the details";
  }else{
    $query = "SELECT * FROM ' . {$_SESSION['email']} . ' WHERE type='P'";
    $result = mysqli_query($con,$query);
    $num = mysqli_num_rows($result);
    if($num == 2){
      $q1 = "INSERT INTO Prayaas(Name,CollegeID,Department,Section,Enrollment,Room,Phone) VALUES('$name','$collegeid','$department','$section','$enrollment','$room','$phone')";
      $msg = "You cannot order more than two books. You have already ordered two books.";
    }elseif($num == 1){
      $data = mysqli_fetch_array($result);
      $books = $data['Books'];
      if($book == $books){
        $q1 = "INSERT INTO Prayaas(Name,CollegeID,Department,Section,Enrollment,Room,Phone) VALUES('$name','$collegeid','$department','$section','$enrollment','$room','$phone')";
        $msg = "You cannot order a same book twice.";
      }else{
        $q1 = "INSERT INTO Prayaas(Name,CollegeID,Department,Section,Enrollment,Room,Phone) VALUES('$name','$collegeid','$department','$section','$enrollment','$room','$phone')";
        $q2 = "INSERT INTO ' . {$_SESSION['email']} . ' (Books,Type) VALUES('$book','P')";
        $msg = "Your order is placed. Prayaas will contact you soon.";
        ...
        try {
           ...
           $mail->Subject = 'Your Prayaas Books';
           $mail->isHTML (TRUE);
           $mail->Body = '
                <html>
                    <body>
                        <h2>Thank you for ordereing through us, '. $name .'.</h2>
                        <p> You have ordered '. $book .' on rent from Prayaas. </p><br>
                        <p> A representative from Prayaas will contact you soon.</p>
                        <p> Meanwhile, keep browsing our <a href="https://novzo.in">website.</a></p>
                    </body>
                </html>
           ';
            if($mail->send()){
                ...
                $msg2 = "We couldn't send you an email. Please ensure that it is correct.";
            }
        }
        catch (Exception $e)
        {
...
<!-- Function used to shrink nav bar removing paddings and adding black background -->
  ...
  <section class="contact-page-area relative" style="padding-top:50px">
    <div class="container cart-container">
    <div class="box">
      <div class="container">
          <div class="row row-centered">
          <section class="wrapper banner-content" style="padding-top:20px;">
            <p style="color:black"><?php echo $msgr1 ?> <br> <?php echo $msgr2 ?></p>
          </section>
        </div>
      </div>
    </div>
